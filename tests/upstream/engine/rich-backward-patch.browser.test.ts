import type { RichState } from '../../../src/engine/rich.ts'
import { describe, expect, it, vi } from 'vitest'

import { patchRich, prepareRich } from '../../../src/engine/rich.ts'
...
describe('rich backward text patches', () => {
  it.each(['…', '', ' […] '])(
    ...
    (ellipsis) => {
      ...
      if (!prepared)
        throw new Error('Expected browser rich preparation.')
      ...
      try {
        ...
        if (marker)
          expect(target.lastChild).toBe(marker)
