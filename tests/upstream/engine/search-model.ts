import { defaultWarmExpansionLimit } from '../../../src/engine/search.ts'

export function estimateColdSearchMaxProbeCount(count: number): number {
  return count <= 0 ? 0 : Math.ceil(Math.log2(count + 1))
}

export interface RankAdvance {
  readonly max: number
  readonly min: number
}

export interface TargetRankInterval {
  readonly max: number
  readonly min: number
}

export interface TargetRankIntervalInput {
...
export interface WarmColdProbeCost {
...
export interface WarmColdProbeCostInput {
...
export interface WarmSearchRankRoom {
