...
import { scheduleFontFrame } from '../../../src/engine/font-frame.ts'
import { listenForFontLoads } from '../../../src/engine/layout.ts'
...
it('shares native font delivery and detaches the final subscriber', async () => {
  ...
  const remove = vi.spyOn(document.fonts, 'removeEventListener')
  const a = vi.fn()
  const b = vi.fn()
  stops.push(listenForFontLoads(a), listenForFontLoads(b))
  await ready()
