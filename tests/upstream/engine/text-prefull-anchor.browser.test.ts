import type { TextClampLayoutInput, TextClampResult } from '../../../src/engine/text.ts'
import { afterEach, expect, it } from 'vitest'
import { fitsContent } from '../../../src/engine/layout.ts'
import { displayTextForKeptCount, prepareText, searchTextLayout } from '../../../src/engine/text.ts'
import { InlineClamp } from '../../../src/index.ts'
import { createApp, h, nextTick, ref } from '../react-adapter.ts'
...
it('checks bare full text after a rejected anchor and refreshes its moved height bounds', async () => {
  ...
  if (anchor.done)
    throw new Error('Expected current-candidate measurement')
  ...
  if (full.done)
    throw new Error('Expected full-source measurement')
