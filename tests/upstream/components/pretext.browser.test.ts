import type { ClampBoundary } from '../../../src/index'
import type { MountedClamp } from '../browser'
import type { Component } from '../react-adapter'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { createPretextLineClampPredictor } from '../../../src/engine/pretext/predictor.ts'
import { LineClamp as StandardLineClamp } from '../../../src/index'
...
import { createApp, defineComponent, h, ref } from '../react-adapter'
...
function trackGeometryReads(elements: readonly unknown[]): () => number {
  ...
  for (const element of elements) {
    if (!(element instanceof HTMLElement))
      throw new Error('Expected clamp geometry.')
...
describe('pretext LineClamp', () => {
  ...
  it('does not pass its predictor to LineClamp instances rendered by slots', async () => {
    ...
    expect(
      (nestedRoot?.querySelector('[data-part="content"]') as HTMLElement | null)?.style.webkitLineClamp,
    ).toBe('')
  })
    ...
    globalThis.ResizeObserver = new Proxy(OriginalResizeObserver, {
      construct(Target, [callback]: ConstructorParameters<typeof ResizeObserver>) {
        return new Target((entries, observer) => {
          ...
          if (clamp)
            deliveredText.push(visibleText(clamp))
        })
      },
    })
  ...
  it('keeps one visible text node through repeated predictive resizes', async () => {
    ...
    if (!(visible instanceof HTMLElement) || !(textNode instanceof Text)) {
      throw new TypeError('Expected a visible text node.')
    }
    ...
    const observer = new MutationObserver((records) => {
      for (const record of records) {
        ...
        if (record.type === 'characterData')
          characterDataRecords.push(record)
      }
    })
