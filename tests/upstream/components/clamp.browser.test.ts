import type { PreparedRich, RichClampResult, RichState, RichStateRank } from '../../../src/engine/rich.ts'
import type { ClampHandle as LineClampExposed, ClampHandle as RichLineClampExposed } from '../../../src/index'
import type { RankAdvance, TargetRankInterval, WarmColdDecision } from '../engine/search-model'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { clampRich, patchRich, prepareRich, rankRichState, richStateForRank } from '../../../src/engine/rich.ts'
import { richWarmExpansionLimit } from '../../../src/engine/search.ts'
import { InlineClamp, LineClamp, RichLineClamp } from '../../../src/index'
...
import {
  ...
  warmSearchDecision,
} from '../engine/search-model'
import { Comment, createApp, defineComponent, h, nextTick, ref } from '../react-adapter'
...
interface RichClampFixtureOptions {
...
function lineContentElement(root: HTMLElement): HTMLElement {
  ...
  if (!(content instanceof HTMLElement)) {
    throw new TypeError('Expected line clamp content element.')
  }
...
function inlineBodyElement(root: HTMLElement): HTMLElement {
  ...
  if (!(body instanceof HTMLElement)) {
    throw new TypeError('Expected inline clamp body element.')
  }
...
interface RichWarmDecisionCheck {
  readonly decision: WarmColdDecision | null
...
describe('lineClamp browser contract', () => {
  ...
  it('emits update:expanded when the exposed toggle is called', async () => {
    ...
    const mounted = mountClamp({
      text: 'abcdefghijklmno',
      props: {
        'onUpdate:expanded': function (value: boolean) {
          values.push(value)
        },
      },
    })
  ...
  it('preserves rich text nodes when reclamping to a same-node whitespace cut', async () => {
    ...
    const target = document.createElement('span')
    const state: RichState | null = patchRich(
      prepared,
      target,
      null,
  ...
  it('preserves rich prefix nodes when clamping from full rich content', () => {
    ...
    const target = document.createElement('span')
    const state: RichState | null = patchRich(prepared, target, null, { kind: 'full' }, '…')
  ...
  it('preserves complete rich prefix nodes when growing to full rich content', () => {
    ...
    patchRich(prepared, target, state, { kind: 'full' }, '…')
  ...
  it('places the rich ellipsis outside the inline element that contains the cut', async () => {
    ...
    if (!(code instanceof HTMLElement)) {
      throw new TypeError('Expected retained code element.')
    }
  ...
  it('uses inert image sources in the hidden rich probe', async () => {
    ...
    if (!(probeImage instanceof HTMLImageElement)) {
      throw new TypeError('Expected the hidden rich probe image.')
    }
  ...
  it('preserves every grapheme rank across nested leaves, comments, and atomic runs', () => {
    ...
    try {
      ...
      if (!searchIndex)
        throw new Error('Expected a supported rich search index.')
      const points = [
        { path: [], offset: 0 },
  ...
  it('rebuilds rich search metadata when a nested CSSOM rule makes atomic wrappers inline', async () => {
    ...
    try {
      ...
      if (!(mediaRule instanceof CSSMediaRule)) {
        throw new TypeError('Expected test media rule to be available.')
      }
  ...
  it('rebuilds rich search metadata when a CSSOM rule display changes in place', async () => {
    ...
    try {
      ...
      if (!(rule instanceof CSSStyleRule)) {
        throw new TypeError('Expected test style rule to be available.')
      }
