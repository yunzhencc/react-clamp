...
import { searchLineEndCandidates } from '../../../src/engine/line/end-search.ts'
import { displayTextForKeptCount, prepareText, setElementText } from '../../../src/engine/text.ts'
...
import { h, ref } from '../react-adapter'
...
function longestPrefix(root: HTMLElement, source: string, marker: string, lines: number): string {
  ...
  try {
    ...
    if (fitsContent(probe, content, lines, undefined))
      return source
    for (let kept = prepared.boundaryOffsets.length - 2; kept >= 0; kept--) {
      ...
      if (fitsContent(probe, content, lines, undefined) || kept === 0)
        return candidate
    }
    return marker
  }
