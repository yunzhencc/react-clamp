import type { ClampState as ClampSlotProps, WrapClampState as WrapClampSlotProps } from '../../../src/index'
import type { WrapClampItemSlotProps } from '../fixture-types'
import type { App, Component } from '../react-adapter'
import { afterEach, describe, expect, it } from 'vitest'
import { InlineClamp, LineClamp, RichLineClamp, WrapClamp } from '../../../src/index'
import { LineClamp as PretextLineClamp } from '../../../src/pretext'
import { settle } from '../browser'
import { createApp, h, ref } from '../react-adapter'
...
function fixture({ coupling }: { coupling?: 'wrap' | 'rich' } = {}) {
  ...
  function snapshot(root: HTMLElement, index: number) {
    // React inserts a newly added attribute after existing attributes. Attribute
    // order is not DOM semantics; retain every name/value while canonicalizing
    // serialization so fresh mounts and updates compare the same whole tree.
    const node = (kinds[index % kinds.length] === 'inline' ? root : root.firstElementChild!)
