import type { TextClampResult } from '../../../src/engine/text.ts'
import { afterEach, expect, it } from 'vitest'
import { prepareText, searchTextLayout } from '../../../src/engine/text.ts'
import { InlineClamp } from '../../../src/index'
import { createApp, h, nextTick, ref } from '../react-adapter'

type Family = 'line' | 'inline'
interface State { width: number, occupancy: number }
interface Fixture {
  text: () => string
...
function enumerate(family: Family, source: string, marker: string, state: State) {
  ...
  try {
    ...
    for (let kept = source.length - 1; kept >= 0; kept--) {
      ...
      if (fits(marked))
        return { fullFits, lastMarkedFits, marked, kept }
    }
