import type { ClampState as LineClampSlotProps } from '../../../src/index'
import type { App } from '../react-adapter'
import { afterEach, describe, expect, it } from 'vitest'
import { displayTextForKeptCount, prepareText } from '../../../src/engine/text.ts'
import { LineClamp } from '../../../src/index'
import { accessibleTextElement, naturalLineCount, settle, textElement } from '../browser'
import { createApp, h, nextTick, ref } from '../react-adapter'
  ...
  try {
    ...
    for (const offsets of [prepared.boundaryOffsets, prepared.fallbackBoundaryOffsets]) {
      ...
      for (let rank = 0; rank < offsets.length; rank++) {
        ...
        if (maxLines !== undefined && naturalLineCount(probe) > maxLines)
          continue
        if (maxHeight !== undefined) {
          ...
          if (content.top < top - 0.5 || content.bottom > top + probe.clientHeight + 0.5)
            continue
        }
        best = text
        kept = rank
