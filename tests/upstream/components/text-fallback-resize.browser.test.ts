...
interface State { width: number, text: string, ellipsis: string, ratio: number, fontSize: number }
...
function enumerateAscii(family: Family, state: State): string {
  ...
  const fits = (candidate: string) => {
    ...
    if (family === 'inline')
      return root.scrollWidth <= state.width + 0.5
    const tops: number[] = []
  ...
  describe(`${family} real word fallback`, () => {
    ...
    it('returns to whole-word cuts when a wider layout can fit them', async () => {
      ...
      for (const width of [72, 105, 150, 900, 100, 4000]) {
        ...
        if (width === 4000)
          expect(warm.text()).toBe(state.text)
      }
    })
