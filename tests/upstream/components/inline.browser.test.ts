import type { ClampBoundary, InlineClampProps, ClampLocation as LineClampLocation } from '../../../src/index'
import type { App, Ref } from '../react-adapter'
import { afterEach, describe, expect, it } from 'vitest'
import { displayTextForKeptCount, normalizeLocationRatio, prepareText } from '../../../src/engine/text.ts'
import { InlineClamp } from '../../../src/index'

import { frame } from '../browser'
import { createApp, defineComponent, h, nextTick, ref } from '../react-adapter'

type InlineClampSplit = NonNullable<InlineClampProps['split']>

interface MountedInlineClamp {
  app: App
...
function rootElement(container: HTMLElement): HTMLElement {
  ...
  if (!(root instanceof HTMLElement)) {
    throw new TypeError('Expected inline clamp root element.')
  }
...
describe('inlineClamp browser contract', () => {
  ...
  it('restores body text when the parent width grows', async () => {
    ...
    if (!(host instanceof HTMLElement)) {
      throw new TypeError('Expected inline clamp host element.')
    }
