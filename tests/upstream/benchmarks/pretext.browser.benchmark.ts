import type { PreparedText, TextClampResult } from '../../../src/engine/text'
import type { ClampBoundary } from '../../../src/engine/types'
import type { App } from '../react-adapter'
import { clearCache, prepareWithSegments } from '@chenglou/pretext'
import { Profiler } from 'react'
import { afterEach, describe, expect, it } from 'vitest'
import { clampPreparedLine, prepareLineClamp } from '../../../src/engine/pretext/clamp'
import { clampTextToLayout, prepareText, setElementText } from '../../../src/engine/text'

import { LineClamp } from '../../../src/pretext'
import { createApp, defineComponent, h, nextTick } from '../react-adapter'
...
const lineLimit = 3
const lineHeight = 22
const repetitions = benchmarkSmoke ? 1 : 5
const coreCycles = 200
const preparationCount = 1_000
const scaleInstances = 200
const scaleRepetitions = benchmarkSmoke ? 1 : 3
...
function runBrowser(scenario: Scenario, widths: readonly number[], prepared: PreparedText): Run {
  ...
  if (initialWidth === undefined)
    throw new Error('Expected an initial width.')
  ...
  try {
    ...
    if (!hint)
      throw new Error(`Could not initialize ${scenario.name}.`)
    ...
    for (const width of widths.slice(1)) {
      ...
      if (!hint)
        throw new Error(`Could not clamp ${scenario.name} at ${width}px.`)
...
function verifyPretextFits(
  ...
  try {
    for (let index = 0; index < texts.length; index += 1) {
      ...
      if (width === undefined)
        throw new Error('Expected a matching width.')
      host.root.style.width = `${width}px`
...
async function runScaleScenario(
      ...
      h(
        ...
        Array.from({ length: scaleInstances }, (_, index) =>
          h(Profiler, {
            id: `pretext-${index}`,
            key: index,
            // Upstream VNode updates translate to actual React subtree commits.
  ...
  try {
    ...
    if (!(host instanceof HTMLElement))
      throw new Error('Expected scale benchmark host.')
  ...
  it('reports component scheduling cost at scale', async () => {
    ...
    for (const pattern of scalePatterns) {
      const changes = pattern.widths
        .slice(1)
        .filter((width, index) => width !== pattern.widths[index])
        .length
