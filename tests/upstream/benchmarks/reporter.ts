...
const prefixes = [
  "TEXT_BENCHMARK", "RICH_BENCHMARK", "WRAP_BENCHMARK", "INVALIDATION_BENCHMARK",
  "UPDATE_WORK_RESULT", "PRETEXT_BENCH_RESULT", "PRETEXT_SCALE_BENCH_RESULT",
  "PRETEXT_PREPARE_BREAKDOWN", "PRETEXT_COLD_MOUNT_RESULT",
] as const;
type Payloads = Partial<Record<(typeof prefixes)[number], unknown>>;

function numericRunSeries(value: unknown, path = "", output = new Map<string, number[]>()) {
  ...
  for (const [key, entry] of Object.entries(object)) {
    if (key !== "runs") numericRunSeries(entry, `${path}/${key}`, output);
  }
  return output;
}
...
export default class UpstreamBenchmarkReporter implements Reporter {
  ...
  onUserConsoleLog(log: { content: string }) {
    for (const prefix of prefixes) {
      ...
      if (index !== -1) this.payloads[prefix] = JSON.parse(log.content.slice(index + marker.length).trim());
    }
  }
  ...
  onTestRunEnd(_modules: unknown, _errors: unknown, reason: TestRunEndReason) {
    ...
    writeFileSync(destination, JSON.stringify({
      ...
      subject: "react-clamp", mode: process.env.UPSTREAM_BENCHMARK_SMOKE === "1" ? "smoke" : "full",
      status: reason, payloads: this.payloads, baseline: baselinePath ?? null, comparisons,
      expectedWorkloads: 114, observedWorkloads, workloadCounts: counts,
    ...
    console.log(`Upstream benchmark report: ${destination}`);
