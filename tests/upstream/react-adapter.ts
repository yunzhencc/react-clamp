...
export type DefineComponent<_T = unknown> = React.ElementType
...
export function ref<T>(initial: T): Ref<T> {
  ...
  const wrap = (input: any): any => {
    if (!input || typeof input !== 'object'
      || (Object.getPrototypeOf(input) !== Object.prototype && !Array.isArray(input)) || React.isValidElement(input)) {
      return input
    }
    if (proxies.has(input))
      return proxies.get(input)
    const proxy = new Proxy(input, {
      get(target, key) { return wrap(Reflect.get(target, key)) },
      set(target, key, next) {
        const changed = !Object.is(Reflect.get(target, key), next)
        const success = Reflect.set(target, key, next)
        if (changed) {
          result.version++
  ...
  result = {
    ...
    listeners: new Set(),
    get value() {
      collecting?.add(result as Ref<unknown>)
      return value
    },
    set value(next: T) {
...
function useTracked(render: () => ReactNode): ReactNode {
  ...
  dependencies.add = (dependency) => {
    dependency.listeners.add(update)
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      dependencies.forEach(dependency => dependency.listeners.delete(update))
      updates.delete(update)
    }
  })
  return node
}
export function defineComponent(input: { setup?: (...args: any[]) => () => ReactNode, render?: () => ReactNode, name?: string } | (() => () => ReactNode)): (props: Record<string, unknown>) => ReactNode {
  const options = typeof input === 'function' ? { setup: input } : input
  function FixtureHost(props: Record<string, unknown>): ReactNode {
    const [render] = useState(() => options.setup?.(props) ?? options.render!)
    return useTracked(render)
  }
  return FixtureHost
}
...
export function createApp(component: Component | { setup?: () => () => ReactNode, render?: () => ReactNode }): App {
  ...
  return {
    mount(container) {
      ...
      flushSync(() => root!.render(React.createElement(Host)))
    },
    unmount() {
...
const slotCallbacks = new WeakMap<(value: any) => ReactNode, { owner: Set<Ref<unknown>> | null, dependencies: Map<Ref<unknown>, number>, run: (value: any) => ReactNode }>()
const callbacks = new WeakMap<object, (value: unknown) => void>()
function css(value: unknown): React.CSSProperties | undefined {
  if (typeof value !== 'string')
    return value as React.CSSProperties | undefined
...
export function h(type: Component | string | typeof Comment, props?: any, children?: any): ReactNode {
  ...
  if (attributes.style !== undefined)
    attributes.style = css(attributes.style)
  if (attributes.innerHTML !== undefined) {
    attributes.dangerouslySetInnerHTML = { __html: attributes.innerHTML }
    delete attributes.innerHTML
  }
  if (attributes.onClampchange) {
    attributes.onClampChange = attributes.onClampchange
    delete attributes.onClampchange
  }
  if (attributes['onUpdate:expanded']) {
    attributes.onExpandedChange = attributes['onUpdate:expanded']
    delete attributes['onUpdate:expanded']
  }
  if (attributes.ref && typeof attributes.ref === 'object' && 'assign' in attributes.ref) {
    ...
    if (!callbacks.has(target))
      callbacks.set(target, value => target.assign(value))
  ...
  if (typeof type !== 'string' && children && typeof children === 'object' && !Array.isArray(children) && !React.isValidElement(children)) {
    ...
    const bind = (callback: any): ((value: any) => ReactNode) | undefined => {
      if (!callback)
        return undefined
  ...
  return committed
    ? React.createElement(React.Profiler, { id: 'upstream-fixture', onRender: (_, phase) => {
        if (phase !== 'mount')
          committed()
      } }, element)
    : element
