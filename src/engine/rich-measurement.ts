import type { PreparedRich, RichSearchIndex, RichState } from './rich.js'
import type { ClampBoundary, ClampLength } from './types.js'
import { prepareSharedRich } from './rich-preparation.js'
import { canSafelyCloneRichProbe, clampRich, patchRich } from './rich.js'
...
interface RichMeasurementInput {
  html: string
...
export function createRichMeasurement(): {
  ...
  restore: (body: HTMLElement, html: string) => void
  ...
  function restore(body: HTMLElement, html: string): void {
  ...
  return {
    ...
    measure(input: RichMeasurementInput): { clamped: boolean, fallback: boolean } {
      ...
      if (!input.probeRoot.hasAttribute('inert'))
        input.probeRoot.setAttribute('inert', '')
      ...
      if (!prepared)
        return { clamped: false, fallback: false }
      ...
      if (signature !== affixSignature) {
        ...
        if (input.before)
          children.push(input.before.cloneNode(true) as HTMLElement)
        children.push(probeBody)
        if (input.after)
          children.push(input.after.cloneNode(true) as HTMLElement)
