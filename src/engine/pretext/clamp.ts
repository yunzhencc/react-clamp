...
import type { PreparedText } from '../text.js'
...
interface LineClampResult {
  readonly clamped: boolean
  readonly text: string
}

interface PreparedLineClamp {
...
function browserLineEnd(line: LayoutLineRange, hasBefore: boolean): LayoutCursor {
  if (!hasBefore || line.end.graphemeIndex === 0)
    return line.end
...
export function clampPreparedLine(
  ...
  for (let line = 0; line < maxLines; line += 1) {
    ...
    if (range.width > lastLineWidth + 0.5) {
      if (line === 0 && before > 0 && line < maxLines - 1)
        continue
      break
    }
