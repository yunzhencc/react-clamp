...
export function textLayoutMetricKey(style: CSSStyleDeclaration): string {
  ...
  const base = font.includes(' ')
    ? `font\n${font}`
    : `longhands\n${font}\n${style.fontFamily ?? ''}\n${style.fontSize ?? ''}\n${style.fontStretch ?? ''}\n${style.fontStyle ?? ''}\n${style.fontWeight ?? ''}\n${style.lineHeight ?? ''}`
  return `${base}\n${style.fontFeatureSettings ?? ''}\n${style.fontKerning ?? ''}\n${style.fontOpticalSizing ?? ''}\n${style.fontSizeAdjust ?? ''}\n${style.fontSynthesis ?? ''}\n${style.fontVariant ?? ''}\n${style.fontVariationSettings ?? ''}\n${style.letterSpacing ?? ''}\n${style.textTransform ?? ''}\n${style.wordSpacing ?? ''}\n${style.getPropertyValue('font-language-override') ?? ''}\n${style.textIndent ?? ''}\n${style.tabSize ?? ''}\n${style.whiteSpace ?? ''}\n${style.lineBreak ?? ''}\n${style.textWrapStyle ?? ''}\n${style.getPropertyValue('text-autospace') ?? ''}\n${style.wordBreak ?? ''}\n${style.overflowWrap ?? ''}\n${style.hyphens ?? ''}\n${style.direction ?? ''}\n${style.unicodeBidi ?? ''}\n${style.writingMode ?? ''}\n${style.textOrientation ?? ''}\n${style.verticalAlign ?? ''}`
}
...
export interface BorderBoxSizeSnapshot {
...
export interface VisibleBoundsCache {
  bottom?: number
  clientTop?: number
  height?: number
...
export interface SimpleLineFit {
  lineHeight: number
  lineStep?: number
...
function collectLineBoxes(rects: DOMRectList): { lines: LineBox[], maxHeight: number } {
  ...
  for (let index = 0; index < rects.length; index += 1) {
    ...
    if (previous && sameLineBox(previous, rect))
      continue
    ...
    if (rect.top <= maxTop + 0.5 && lines.some(line => sameLineBox(line, rect)))
      continue
