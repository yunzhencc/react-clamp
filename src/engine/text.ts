...
import type { ClampBoundary, ClampLength, LineClampLocation } from './types.js'

import { fitsContent } from './layout.js'
import { defaultWarmExpansionLimit, searchFittingIndex } from './search.js'
...
export interface TextClampFitInput {
...
export interface TextClampLayoutInput {
...
function graphemeBoundaryOffsets(text: string, firstUnsafe: number): number[] {
  if (firstUnsafe >= 64)
    return [0, ...iterateGraphemeBoundaryOffsets(text)]
...
function wordBoundaryOffsets(
  ...
  for (const part of wordSegmenter.segment(text)) {
    ...
    if (!unitSafe) {
      while ((fallbackBoundaryOffsets[fallbackIndex] ?? Infinity) < offset) {
        fallbackIndex += 1
      }
    }
...
function prepareWordText(text: string): PreparedText {
  ...
  if (boundaryOffsets.at(-1) !== text.length)
    boundaryOffsets.push(text.length)
...
export function matchingTextClampHint(
  ...
  if (!hint || !sameTextClampContext(hint, context))
    return null
  return (
    hint.fullPrepared
...
export function shouldRecheckFullTextFit(
  ...
  if (!hint || hint.boundaryOffsets !== result.boundaryOffsets)
    return false
...
export function* searchTextCandidates({
  ...
  function* fitsKeptCount(kept: number): Generator<string, boolean, boolean> {
    ...
    if (kept < boundaryCount) {
      if (candidate === fittingText)
        return true
      if (candidate === failingText)
        return false
    }
