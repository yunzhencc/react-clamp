import type { PreparedText, TextClampContext, TextClampHint, TextClampResult } from './text.js'
import { isContentIndependentWidth, textLayoutMetricKey } from './layout.js'
import { warmSearchLocalCoverage } from './search.js'
import { canSkipFullTextFit, displayTextForKeptCount, estimateTextRankFromFull, fallbackSearchPrepared, fullTextClampResult, matchingTextClampHint, nextClampedMaxWidth, searchTextCandidates, setElementText, shouldRecheckFullTextFit } from './text.js'

interface InlineSearchInput { root: HTMLElement, target: HTMLElement, prepared: PreparedText, ellipsis: string, ratio: number, split: unknown, rootWidth?: number }
...
export function createInlineSearch(): {
  search: (input: InlineSearchInput) => Generator<() => number, string | null, number>
  invalidate: () => void
} {
