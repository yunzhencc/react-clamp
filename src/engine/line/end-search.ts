...
function* searchWords(
  ...
  for (let word = step.value; word >= 0; word--) {
    ...
    if (candidate.value >= 0)
      return resultFor(input, low + candidate.value)
  }
  return resultFor(input, 0)
}
...
function* searchCharacterBound(
  ...
  try {
    ...
    if (style === null)
      context.target.removeAttribute('style')
    ...
    yield {
      ...
      read: () => {
        ...
        while (low < high) {
          ...
          if (rects.length > 0 && rects[rects.length - 1]!.top > ceiling + 0.5)
            high = middle
          else low = middle + 1
        }
        upper = Math.min(
...
export function* searchLineEndCandidates(
  ...
  if (data.cursive || cursive.test(input.ellipsis))
    return yield* searchTextCandidates(input)
