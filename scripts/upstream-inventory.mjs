...
function inspect(path) {
  ...
  const category = name.endsWith('.benchmark.ts')
    ? 'browser-benchmark'
    : name.endsWith('.test.ts')
      ? name === 'demo-page.browser.test.ts'
        ? 'website-browser-test'
        : /(?:template-ref|wrap-render)-source/.test(name)
          ? 'vue-source-contract-test'
          : name.endsWith('.browser.test.ts') ? 'library-browser-test' : 'node-test'
      : ['type-surface.ts', 'wrap-slot-types.vue'].includes(name)
          ? 'type-contract'
          : name === 'env.d.ts'
  ...
  function arrayElements(node, seen = new Set()) {
    ...
    if (ts.isCallExpression(node) && ts.isPropertyAccessExpression(node.expression)) {
      ...
      if (['map', 'flatMap'].includes(method)) {
        ...
        if (inputs && callback && ts.isArrowFunction(callback) && !ts.isBlock(callback.body)) {
          ...
          if (outputs)
            return inputs.flatMap(input => outputs.map(entry => `binding ${text(callback.parameters[0]?.name)} = ${input}: ${entry}`))
        }
      }
    }
    ...
    for (const item of node.elements) {
      if (ts.isSpreadElement(item)) {
        const values = arrayElements(item.expression, seen)
        if (!values)
          return null
        elements.push(...values)
      }
      else {
        elements.push(text(item))
      }
    }
    return elements
  }
  function callInfo(node) {
    ...
    if (ts.isCallExpression(expr) && ts.isPropertyAccessExpression(expr.expression) && ['each', 'for'].includes(expr.expression.name.text)) {
      ...
      parameterization = expr.expression.name.text
    ...
    if (!ts.isIdentifier(expr) || !['it', 'test', 'describe', 'suite', 'bench'].includes(expr.text))
      return null
  ...
  function walk(node, suites = [], loops = []) {
    ...
    if (ts.isForOfStatement(node) || ts.isForInStatement(node) || ts.isForStatement(node)) {
      ...
      walk(node.statement, suites, [...loops, loop])
      return
    }
  ...
  if (category === 'browser-benchmark') {
    ...
    const visit = (node) => {
      if (ts.isPropertyAssignment(node) && ['name', 'scenario'].includes(text(node.name)) && ts.isStringLiteralLike(node.initializer))
        result.benchmarkSource.namedScenarios.push({ name: node.initializer.text, sourceLine: line(node) })
      if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) && node.initializer && ts.isNumericLiteral(node.initializer))
        result.benchmarkSource.numericConfiguration[node.name.text] = Number(node.initializer.text)
      if (ts.isTypeAliasDeclaration(node) && /Metrics|Run/.test(node.name.text))
        result.benchmarkSource.metricTypes.push({ name: node.name.text, sourceLine: line(node), source: text(node) })
...
if (args.includes('--write')) {
  ...
  for (const file of inventory.files) {
    ...
    for (const entry of file.declarations) {
      const old = previous.declarations.find(item => item.id === entry.id)
      if (old)
        entry.correspondence = old.correspondence
    }
  }
