...
const tokens=text=>{const scanner=ts.createScanner(ts.ScriptTarget.Latest,true,ts.LanguageVariant.Standard,text);const result=[];let token;for(token=scanner.scan();token!==ts.SyntaxKind.EndOfFileToken;token=scanner.scan())result.push([token,scanner.getTokenText()]);return JSON.stringify(result)};
function inspect(path){
 const raw=readFileSync(path,'utf8');const source=ts.createSourceFile(path,raw,ts.ScriptTarget.Latest,true);const declarations=[];const assertions=[];
 const line=node=>source.getLineAndCharacterOfPosition(node.getStart(source)).line+1;
 const text=node=>node.getText(source);
 function rootCall(expr){while(ts.isPropertyAccessExpression(expr)||ts.isElementAccessExpression(expr)||ts.isCallExpression(expr))expr=expr.expression;return ts.isIdentifier(expr)?expr.text:null;}
 function visit(node){
  if(ts.isCallExpression(node)){
   const root=rootCall(node.expression);
   if(root==='expect') {assertions.push({line:line(node),expression:text(node),sha256:hash(tokens(text(node)))});return;}
 ...
 visit(source);return {sha256:hash(raw),declarations,assertions};
}
const components=[];
for(const name of readdirSync('tests/upstream/components').filter(n=>n.endsWith('.browser.test.ts')).sort()){
 const original=`packages/vue-clamp/tests/${name}`;const local=`tests/upstream/components/${name}`;
 const a=inspect(join(upstream,original)),b=inspect(local);
...
if (mixed) mixed.translation += ' Snapshot helper clones the tree and sorts attribute names lexically before serialization: React insertion order may vary, while all attribute names/values and DOM hierarchy are retained.';
...
if(write)writeFileSync('tests/upstream/components/mapping.json',JSON.stringify(componentManifest,null,2)+'\n');
...
for(const f of components)add(f.original,f.local,'tests/upstream/components/mapping.json',f.translation,f.declarations.map(d=>({originalLine:d.originalLine,localLine:d.localLine})));
...
for(const f of engine.suiteMappings){
 const original=inspect(join(upstream,f.original)),local=inspect(f.local);
...
for(const kind of ['node','benchmarks']){
 const manifest=`tests/upstream/${kind}/mapping.json`;const data=JSON.parse(readFileSync(manifest,'utf8'));
 for(const f of data.files)add(f.upstreamFile,f.adaptedFile,manifest,f.translation,(f.tests??f.declarations??[]).map(t=>({originalLine:t.upstream.line,localLine:t.adapted.line})));
}
const demoPath='tests/upstream/demo/mapping.json';const demo=JSON.parse(readFileSync(demoPath,'utf8'));
...
for(const original of demo.typeContracts.source)add(`packages/vue-clamp/tests/${original}`,demo.typeContracts.localFile,demoPath,`${demo.typeContracts.adaptations.join('; ')}. Checked by strict TypeScript; public generic JSX acceptance/rejection cases retained. Pending semantic differences: ${demo.typeContracts.pending.join('; ')}`);
...
for(const file of inventory.files){
 ...
 if(!mapped.length){unresolved.push(file.path);continue;}
 ...
 for(const declaration of file.declarations){
  ...
  if(!matching.length){unresolved.push(declaration.id);continue;}
