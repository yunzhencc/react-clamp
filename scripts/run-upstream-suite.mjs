...
import { root, groups, inputs, configuredBrowsers } from './upstream-inputs.mjs';
const [group, ...extra] = process.argv.slice(2);
if (!groups.includes(group)) throw new Error(`Usage: node scripts/run-upstream-suite.mjs ${groups.join('|')} [runner arguments]`);
