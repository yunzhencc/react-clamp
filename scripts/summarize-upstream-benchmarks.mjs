...
const expected = {
  TEXT_BENCHMARK: 17, RICH_BENCHMARK: 12, WRAP_BENCHMARK: 27,
  INVALIDATION_BENCHMARK: 2, UPDATE_WORK_RESULT: 10,
  PRETEXT_BENCH_RESULT: 39, PRETEXT_SCALE_BENCH_RESULT: 2,
  PRETEXT_PREPARE_BREAKDOWN: 3, PRETEXT_COLD_MOUNT_RESULT: 2,
};
...
for (const [key, runs] of Object.entries({ TEXT_BENCHMARK: 5, RICH_BENCHMARK: 4, WRAP_BENCHMARK: 5, INVALIDATION_BENCHMARK: 5 })) {
  ...
  for (const scenario of payloads[key].scenarios) assert.equal(scenario.summary.runs.length, runs, `${key} repetitions`);
}
...
for (const scenario of Object.values(payloads.PRETEXT_PREPARE_BREAKDOWN.paths)) assert.equal(scenario.runs.length, 5);
...
for (const group of Object.keys(expected)) {
  ...
  if (payload.scenarios) {
  ...
  } else if (group === 'UPDATE_WORK_RESULT') {
  ...
  } else if (group === 'PRETEXT_PREPARE_BREAKDOWN') {
    ...
    for (const scenario of payload) {
      ...
      } else if (group === 'PRETEXT_BENCH_RESULT') {
...
const report = {
  upstreamSha: metrics.upstreamSha, subject: 'react-clamp', mode: 'full', status: 'passed',
  matrixComplete: true, repetitionCountsVerified: true,
  fileCount: 6, testDeclarations: 9, expectedWorkloads: 114, observedWorkloads: 114, skippedWorkloads: 0,
  generatedAt: new Date().toISOString(), command: 'pnpm run test:upstream:benchmarks',
...
const round = value => {
  if (typeof value === 'number') { assert.ok(Number.isFinite(value)); return Math.round(value * 10_000) / 10_000; }
  if (Array.isArray(value)) return value.map(round);
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, entry]) => [key, round(entry)]));
  return value;
};
